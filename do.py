...
def detection_target_layer(batch_proposals, batch_mrcnn_class_scores, batch_gt_class_ids, batch_gt_boxes, cf):
    ...
    for b in range(len(batch_gt_class_ids)):
        #print("batch_gt_class_ids[b]",batch_gt_class_ids[b])
...
class net(nn.Module):
    ...
    def train_forward(self, batch, is_validation=False):
        ...
        #print("forward gt_class_ids shape",gt_class_ids.shape)
...
# np_anchors = mutils.generate_pyramid_anchors(cf)
# anchors = torch.from_numpy(np_anchors).float().cuda()


# img=torch.rand(1,1,32,512,512).cuda()
...
# print(anchors.shape)
# print(proposal_count)
# batch_rpn_rois, batch_proposal_boxes = proposal_layer(rpn_pred_probs, rpn_pred_deltas, proposal_count, anchors, cf)
...
# detection_boxes = detections[:, :cf.dim * 2 + 1] / scale
# # with torch.no_grad():
    # # detection_masks = mask(mrcnn_feature_maps, detection_boxes)



# proposal_boxes=batch_proposal_boxes
# mrcnn_class_logits, mrcnn_pred_deltas, mrcnn_pred_mask, target_class_ids, mrcnn_target_deltas, target_mask,  \
# sample_proposals = loss_samples_forward(gt_class_ids, gt_boxes)